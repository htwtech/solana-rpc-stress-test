...
- **Оптимизированная сборка**: Release сборка включает:
  - `lto = true` - Link Time Optimization для межмодульной оптимизации
  - `opt-level = 3` - максимальный уровень оптимизации компилятора
  - `codegen-units = 1` - единый модуль кодогенерации для лучшей оптимизации
  - `panic = "abort"` - мгновенное завершение при панике без размотки стека
  - Эти настройки обеспечивают максимальную производительность за счет увеличенного времени компиляции
...
```bash
# Запуск с конфигом
./target/release/solana-rpc-stress-test -c config.json

# С дополнительными опциями
./target/release/solana-rpc-stress-test -c config.json -v -p
```
...
- `--timeout-ms` / `-t`: Задержка между последовательными запросами каждого воркера в миллисекундах (по умолчанию: 1)
  - Минимальное значение: 1 мс
  - Чем меньше значение, тем выше нагрузка (больше запросов в секунду)
  - **Это НЕ таймаут ожидания ответа** - это интервал между запросами для контроля RPS (requests per second)
...
- `--http-timeout`: Максимальное время ожидания HTTP ответа в секундах (по умолчанию: 30)
  - Применяется к каждому отдельному HTTP запросу
  - При превышении этого времени запрос считается таймаутом и увеличивает счетчик "HTTP timeouts"
  - **Это отличается от `timeout_ms`**, которое контролирует частоту запросов, а не время ожидания
...
Конфигурационный файл использует формат JSON и позволяет:
...
- `timeout_ms`: Задержка между запросами в миллисекундах (опционально) - контролирует частоту запросов
- `duration`: Продолжительность теста в секундах (опционально)
- `http_timeout`: Максимальное время ожидания HTTP ответа в секундах (опционально)
...
**Важно**: Для оптимизации использования памяти, перцентили (p50, p95, p99) рассчитываются на основе выборки из первых 100,000 времен ответов. Если общее количество запросов превышает этот лимит, в выводе появится примечание "Latency stats based on X samples (sampling)". Это не влияет на точность средней (Average), минимальной (Minimum) и максимальной (Maximum) латентности, которые рассчитываются по всем запросам.
...
```bash
# Создайте config.json с нужными методами
./target/release/solana-rpc-stress-test -c config.json

# С ping тестом
./target/release/solana-rpc-stress-test -c config.json -p
```
...
# 3. Скопируйте полученную base64 строку в config.json
```
...
### Ошибка "Blockhash not found" (code: -32002)
...
**Причина:** Транзакция использует устаревший blockhash, который больше не существует в блокчейне.

**Решение:**
1. Получите свежий blockhash через RPC:
   ```bash
   curl -X POST http://YOUR_RPC:8899 \
     -H "Content-Type: application/json" \
     -d '{"jsonrpc":"2.0","id":1,"method":"getLatestBlockhash","params":[{"commitment":"finalized"}]}'
   ```
2. Обновите транзакцию с новым blockhash (требует Solana SDK или инструментов)
3. Переподпишите транзакцию
4. Используйте обновленную транзакцию в конфиге

**Для `simulateTransaction`:**
- Можно использовать `{ "replaceRecentBlockhash": true }` в параметрах, но это **не работает** с `sigVerify: true`
- Если нужна проверка подписей, обновите транзакцию вручную перед тестом

**Для `sendTransaction`:**
- Транзакция должна быть подписана и использовать актуальный blockhash
- Обновите blockhash и переподпишите транзакцию перед использованием
